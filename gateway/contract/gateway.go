...
func (g *FrimaContractGateway) parseLog(vLog types.Log) *model.ContractEvent {
	...
	switch eventSig {
	case g.contractABI.Events["ItemListed"].ID.Hex():
		return g.parseItemListed(vLog)
	case g.contractABI.Events["ItemPurchased"].ID.Hex():
		return g.parseItemPurchased(vLog)
	case g.contractABI.Events["ItemUpdated"].ID.Hex():
		return g.parseItemUpdated(vLog)
	case g.contractABI.Events["ItemCancelled"].ID.Hex():
		return g.parseItemCancelled(vLog)
	case g.contractABI.Events["ReceiptConfirmed"].ID.Hex():
